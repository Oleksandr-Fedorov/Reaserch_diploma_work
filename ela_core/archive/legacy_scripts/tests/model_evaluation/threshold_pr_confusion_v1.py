"""Evaluate model V1 with validation-selected thresholds.

The script prints both Youden-J and F1 threshold candidates on validation, then
uses the Youden-J threshold for the final test-set report.
"""
...
# --- 1. НАСТРОЙКИ ---
MODEL_PATH = "models/23.pth" # Твои текущие веса (где было 69%)
TRAIN_DIR = "D:/Diplom/ela_core/datasets/CASIA_Unified_Randomized/train"
TEST_DIR = "D:/Diplom/ela_core/datasets/CASIA_Unified_Randomized/test"
...
print(f"✅ Оптимальный порог (Youden's J - Для науки): {optimal_threshold:.4f}")

# --- ДОБАВЛЕННЫЙ БЛОК ДЛЯ F1-ПОРОГА ---
from sklearn.metrics import f1_score
f1_scores = [f1_score(val_labels, [1 if p >= t else 0 for p in val_probs]) for t in thresh_v]
f1_optimal_threshold = thresh_v[np.argmax(f1_scores)]
print(f"✅ Оптимальный порог (F1 - Для продакшена/Демо): {f1_optimal_threshold:.4f}")

idx = np.argmax(f1_scores)
for i in range(max(0, idx-2), min(len(thresh_v), idx+3)):
    print(f"t={thresh_v[i]:.4f}  F1={f1_scores[i]:.4f}  J={tpr_v[i]-fpr_v[i]:.4f}")
# -------------------------------------
...
plt.savefig(PLOTS_DIR / "thesis_confusion_matrix_optimal.png", dpi=300)
