# ============================================================
# RGB BASELINE EVALUATION SCRIPT (STANDALONE)
# ============================================================
...
# 1. ЖЕЛЕЗОБЕТОННАЯ ФИКСАЦИЯ ВИПАДКОВОСТІ (Обязательно для точного воссоздания Val-сплита)
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return image, label, os.path.basename(img_path) # Добавлено возвращение имени файла
...
# 4. ПУТИ (Настроено под Kaggle)
KAGGLE_INPUT_DIR = 'D:/Diplom/ela_core/datasets/CASIA_Unified_Randomized_RGB_v1'
train_dir = f'{KAGGLE_INPUT_DIR}/train'
test_dir = f'{KAGGLE_INPUT_DIR}/test'
csv_path = os.path.join(KAGGLE_INPUT_DIR, "dataset_log.csv")
MODEL_PATH = 'D:/Diplom/ela_core/models/model_resnet_rgb_LATEST.pth' # Твоя обученная модель

print("⚙️ Загрузка датасетов и точное восстановление Source-Aware Val-сплита...")
...
rng = random.Random(42) # Тот же seed, тот же порядок!
...
with torch.no_grad():
    for images, labels, _ in val_loader:
        ...
        probs = torch.softmax(outputs, dim=1)[:, 1] # Вероятности класса FAKE
        val_labels.extend(labels.numpy())
        val_probs.extend(probs.cpu().numpy())

fpr_v, tpr_v, thresholds_v = roc_curve(val_labels, val_probs)

# Расчет двух кандидатов на лучший порог
...
# Выбираем порог по F1 (как лучший баланс для продакшена)
...
# ==========================================
print(f"\n🚀 ФАЗА 2: Экзамен на изолированной Test выборке...")
...
# Делаем предсказания по двум порогам для сравнения
...
# Сохраняем новые, выверенные вероятности (для Late Fusion)
predictions_csv_path = "D:/Diplom/ela_core/artifacts/rgb_test_predictions_optimal.csv"
...
plt.title('ROC-AUC Curve (RGB Baseline Test)')
plt.legend(loc="lower right")
plt.grid(alpha=0.3)
plt.savefig('D:/Diplom/ela_core/artifacts/images/plots/model_resnet_rgb_LATEST/rgb_roc_curve.png', dpi=300)
plt.close()
...
plt.title(f'Матрица ошибок — RGB Baseline (Порог: {BEST_THRESHOLD:.2f})')
plt.savefig('D:/Diplom/ela_core/artifacts/images/plots/model_resnet_rgb_LATEST/rgb_confusion_matrix_optimal.png', dpi=300)
plt.close()

print("✅ Графики сохранены: 'D:/Diplom/ela_core/artifacts/images/plots/model_resnet_rgb_LATEST/rgb_roc_curve.png' и 'D:/Diplom/ela_core/artifacts/images/plots/model_resnet_rgb_LATEST/rgb_confusion_matrix_optimal.png'")